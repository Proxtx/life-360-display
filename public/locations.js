...
export const loadFile = async (pwd, file) => {
  ...
  return {
    success: true,
    file: await fs.readFile(config.locations + file, "utf-8"),
  };
};
...
export const getDataInTimespan = async (pwd, start, end) => {
  ...
  for (let i of files) {
    if (!lookingForEnd) {
      if (i > start) {
        filesToLoad.push(startFile ? startFile : i);
        lookingForEnd = true;
        continue;
      }
  ...
  let startLocations = locationData.shift();
  if (startLocations) {
    for (let i of Object.keys(startLocations)) {
      if (i > start) {
        resultObject[i] = startLocations[i];
      }
    }
  }
  if (locationData.length > 0) {
    let endLocations = locationData.pop();
    for (let i of locationData) {
      resultObject = { ...resultObject, ...i };
    }
    if (locationData.length > 0) {
      for (let i of Object.keys(endLocations)) {
        if (i < end) {
          resultObject[i] = startLocations[i];
        }
      }
  ...
  return { success: true, locations: resultObject };
};

export const getFilesInOrder = async () => {
  let dates = [];
  let files = await fs.readdir(config.locations);
  files.forEach((file) => {
    if (file == "locations.txt") return;
...
const getTimesInFileInOrder = (raw) => {
